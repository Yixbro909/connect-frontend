...
import { localCreateRoomEndPoint, localLoginEndPoint } from '../pages/others/endpoints';
...
interface IProps {
    ...
    setOpenCreateRoomModal: (x: boolean) => void
}

const CreateRoomModal: React.FC<IProps> = ({ openCreateRoomModal, setOpenCreateRoomModal }) => {
    ...
    async function fetchUser(email: string, password: string) {
        ...
        try {
            ...
            if (!auth.username) {
                ...
                roomRes = await axios.post(localCreateRoomEndPoint, { room_name: roomName, creator: loginRes.data.username });
                setUser(loginRes.data);
                dispatch(login(loginRes.data))


            } else {
                roomRes = await axios.post(localCreateRoomEndPoint, { room_name: roomName, creator: auth.username });
                setUser(auth)
                dispatch(login(auth))
                ...
                <Box sx={matches ? responsiveStyle : style}>
                    ...
                    <div className="container-fluid">
                        ...
                        <form action="" className='w-full md:w-1/2 mx-auto mt-5 items-center justify-center flex flex-col' onSubmit={handleSubmit}>
                            ...
                            <button disabled={loading} type="submit" className={`${loading ? 'bg-gray-300 text-gray-700' : 'bg-gradient-to-br from-emerald-400'} to-blue-600 transition-all duration-200 self-start outline-none border-0 mt-5 font-bold text-white  rounded-md px-4 py-2`}>{loading ? 'loading ...' : 'Create now'}</button>
